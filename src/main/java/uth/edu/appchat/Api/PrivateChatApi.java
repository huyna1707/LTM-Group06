...
public class PrivateChatApi {
    ...
    public ResponseEntity<?> getMyPrivateChats(Authentication auth) {
        try {
            ...
            List<Map<String, Object>> chatList = chats.stream().map(chat -> {
                ...
                // Lấy streak display
                String streakDisplay = chatStreakService.getPrivateChatStreakDisplay(chat);
    ...
    public ResponseEntity<?> sendPrivateMessage(@PathVariable Long chatId,
        ...
        try {
            ...
            // Cập nhật streak
            ChatStreak streak = chatStreakService.updatePrivateChatStreak(chat);
            ...
            return ResponseEntity.ok(Map.of(
                    ...
                    "streak", streak.getStreakDisplay()
            ));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(Map.of(
    ...
    // Lấy thông tin streak của chat
    @GetMapping("/{chatId}/streak")
    public ResponseEntity<?> getChatStreak(@PathVariable Long chatId, Authentication auth) {
        try {
            User currentUser = userRepository.findByUsername(auth.getName())
                    .orElseThrow(() -> new RuntimeException("User not found"));
            ...
            Optional<ChatStreak> streakOpt = chatStreakService.getPrivateChatStreak(chat);
            ...
            return ResponseEntity.badRequest().body(Map.of(
                    "error", "Failed to get streak info: " + e.getMessage()
            ));
        }
    }
    ...
    public ResponseEntity<?> restoreStreak(@PathVariable Long chatId, Authentication auth) {
        try {
            User currentUser = userRepository.findByUsername(auth.getName())
                    .orElseThrow(() -> new RuntimeException("User not found"));
            ...
            if (restored) {
                return ResponseEntity.ok(Map.of(
                        ...
                        "streak", chatStreakService.getPrivateChatStreakDisplay(chat)
                ));
            } else {
                return ResponseEntity.badRequest().body(Map.of(
            ...
            return ResponseEntity.badRequest().body(Map.of(
                    "error", "Failed to restore streak: " + e.getMessage()
            ));
        }
    }
}